...
def main(unused_argv):
    ...
    path1 = 'BaumbilderBA_augmented'
    path2 = 'BaumbilderBA_very_augmented_farbig_227'  #path of folder to save images DGX
    ...
    for file in listing:
        im = tf.image.decode_png(path1 + '/' + file)
        img = tf.image.resize_images(im, [img_rows,img_cols])
        img.save(path2 +'/' + file, "PNG")

        # random adjustments
        tf.image.random_brightness(img).save(path2 +'/bright_' + file, "PNG")
        tf.image.random_contrast(img).save(path2 +'/contrast_' + file, "PNG")
        tf.image.random_hue(img).save(path2 +'/hue_' + file, "PNG")
        tf.image.random_saturation(img).save(path2 +'/saturation_' + file, "PNG")
