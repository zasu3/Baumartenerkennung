...
path2 = 'BaumbilderBA_augmented_training'  #path of folder to save images DGX
pathValidation = 'BaumbilderBA_augmented_validation'
...
for file in listing:
    ...
    if file == '.DS_Store':
        continue
    im = Image.open(path1 + '/' + file).convert('LA')
...
validationlist = os.listdir(pathValidation)
...
immatrix = array([array(Image.open(path2 + '/' + im2)).flatten()
                  for im2 in imlist],'f')
immatrix_val = array([array(Image.open(pathValidation + '/' + imval)).flatten()
                  for imval in validationlist], 'f')

num_samples = imnbr
num_validation = valnbr
#print(num_samples)
#print(immatrix.shape)


# label
label_train=np.ones((num_samples,),dtype = int)
label_val = np.ones((num_samples,),dtype = int)

get_label(label_train, imlist)
get_label(label_val, validationlist)

def get_label(label, imagelist):
    i=0
...
X_test, y_test = (val_data[0], val_data[1])
...
fully_1 = model.add(Dense(256, activation='relu'))
...
#model.save('08-12-2018_Conv1_15epochs.h5', overwrite=True, include_optimizer=False)
#tf.keras.models.save_model(model, '08-12-18_Conv1_1epochs.h5')
